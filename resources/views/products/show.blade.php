            ...
            <div class="col-md-6 big-picture bordBig">
                @if ($product->images->count())
                    @php
                        ...
                        $imageSrc = Storage::url('images/' . $product->category->category_name . '/' . $product->brand->brand_name . '/' . $defaultImage->filename);
                    @endphp
                    <img id="bigImage" src="{{ $imageSrc }}" class="big-img" alt="Big Product Image" />
                @else
                ...
                <div class="small-picture">
                    @if ($product->images && $product->images->count())
                        @foreach ($product->images as $image)
                            @php
                                $imageSrc = Storage::url('images/' . $product->category->category_name . '/' . $product->brand->brand_name . '/' . $image->filename);
                            @endphp
                            <figure>
